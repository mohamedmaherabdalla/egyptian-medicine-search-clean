#!/usr/bin/env python3
"""Evaluate Algorithms 1-5 on accepted Data 3 OCR-error observations."""
...
RESULT_FIELDS = [
    ...
    "ground_truth_in_egypt_db", "ocr_output_in_egypt_db",
    "ocr_output_catalog_families", "dangerous_collision_families",
    "shared_character_count", "shared_ngram_count", "audit_decision",
    "failure_class", "source_review_requirement",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--algorithms", default="1,2,3,4,5")
...
def prepare_algorithms(selected: set[str], records: list[dict[str, Any]]) -> dict[str, Callable[[str], dict[str, Any]]]:
    ...
    if "2" in selected:
        ...
        runners["algorithm_2_external_fast"] = (
            lambda query, module=module, catalog=catalog: module.search_catalog(
                catalog,
                query,
                20,
            )
    ...
    if "3" in selected:
        ...
        runners["algorithm_3_rank_fusion"] = (
            lambda query, module=module, catalog=catalog: module.search_catalog(
                catalog,
                query,
                20,
    ...
    if "4" in selected:
        ...
        catalog = module.prepare_catalog()
        runners["algorithm_4_family_rescue"] = (
            lambda query, module=module, catalog=catalog: module.search_catalog(
                catalog,
                query,
                20,
    ...
    if "5" in selected:
        module = load_module(
            EVALUATION_DIR / "master_algorithms" / "algorithm_5_commercial_name_search.py",
            "data3_algorithm_5",
        )
        catalog = module.prepare_catalog()
        runners["algorithm_5_evidence_rescue"] = (
            lambda query, module=module, catalog=catalog: module.search_catalog(
                catalog,
                query,
                20,
...
def evaluate_case(case: dict[str, str], algorithm: str, runner: Callable[[str], dict[str, Any]]) -> dict[str, object]:
    ...
    failure_class, source_review = classify_failure(case, first_rank, bool(results))
    return {
        ...
        "ground_truth_in_egypt_db": case.get("ground_truth_in_egypt_db", ""),
        "ocr_output_in_egypt_db": case.get("ocr_output_in_egypt_db", ""),
        "ocr_output_catalog_families": case.get("ocr_output_catalog_families", ""),
        "dangerous_collision_families": case.get("dangerous_collision_families", ""),
        "shared_character_count": case.get("shared_character_count", ""),
        "shared_ngram_count": case.get("shared_ngram_count", ""),
        "audit_decision": "retain_locked_fair_case",
        "failure_class": failure_class,
        "source_review_requirement": source_review,
...
def classify_failure(
    ...
    """Describe observed failure evidence without changing the locked denominator."""

    if first_rank <= 20:
        return "recovered", "none"
    if not has_results:
        return "no_candidates_returned", "algorithm_retrieval_review"
    normalized_distance = float(case.get("normalized_edit_distance") or 0)
    edit_distance = int(float(case.get("edit_distance") or 0))
    shared_characters = int(float(case.get("shared_character_count") or 0))
    if normalized_distance > 0.60 and shared_characters < 3:
        return "extreme_low_character_evidence", "review_original_crop"
    if normalized_distance > 0.60:
        return "extreme_competing_lexical_evidence", "review_original_crop"
    if edit_distance >= 4:
        return "high_distance_candidate_competition", "algorithm_retrieval_review"
    if len(compact_text(case.get("input", ""))) <= 3:
        return "short_query_insufficient_evidence", "review_original_crop"
    return "standard_distance_retrieval_gap", "algorithm_retrieval_review"
