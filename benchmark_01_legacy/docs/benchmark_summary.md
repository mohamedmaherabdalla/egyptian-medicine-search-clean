...
| file | rows | purpose |
| --- | ---: | --- |
| `benchmark_01_legacy/artifacts/01_current_app/case_results.csv.gz` | 341,901 | Lossless full raw result table: one row per test case with rank, top result, top-5 bases, and safety flags. |
...
| `benchmark_01_legacy/artifacts/02_external_fast/case_results.csv.gz` | 341,901 | Lossless full raw result table for the external English fast algorithm. |
...
The complete per-test-case results are in `benchmark_01_legacy/artifacts/01_current_app/case_results.csv.gz`.
