#!/usr/bin/env python3
"""Run classical retrieval baselines, Algorithms 1-5, and Algorithm 4 ablations."""
...
def prepare_existing_algorithms(records: list[dict[str, Any]]) -> list[PreparedRunner]:
    ...
    definitions = {
        ...
        "algorithm_5_evidence_rescue": "Algorithm 5, evidence-guided retrieval, bounded reranking, and conservative clarification.",
    }
    ...
    runners.append(
        PreparedRunner(
            ...
            lambda query: module.search_catalog(catalog, query, TOP_K),
            preparation_ms,
            definitions["algorithm_5_evidence_rescue"],
        )
    )
    return runners
...
def write_report(
    ...
    lines = [
        "# Retrieval Baselines, Algorithm 5, and Algorithm 4 Ablations",
        "",
        "## Evaluation contract",
        "",
    ...
    if retrieval_metrics:
        ...
        a5 = primary_by_name["algorithm_5_evidence_rescue"]
        ...
        lines.extend(
            [
                "",
                "### Paired comparison with Algorithm 5",
                "",
                "Reference-only counts are pairs recovered by A5 but missed by the comparison. Other-only counts are the reverse. The exact McNemar p-value tests whether the discordant counts are balanced; it does not measure effect size. P-values are exploratory and are not adjusted for multiple comparisons.",
                "",
                paired_markdown(retrieval_pairs),
                "",
                (
                    f"A5 exceeds the strongest classical Hit@1 baseline, exhaustive Levenshtein, by "
                    f"{100 * (a5['hit_at_1'] - levenshtein['hit_at_1']):.2f} percentage points. "
                    f"Jaro-Winkler is the strongest classical Hit@20 baseline; A5 leads it by "
                    f"{100 * (a5['hit_at_20'] - jaro['hit_at_20']):.2f} points."
                ),
                "",
                "### Concrete ranking switches",
                "",
                "The reference is Algorithm 5; the other system is exhaustive Levenshtein.",
                "",
                example_markdown(
                    [
                        row
