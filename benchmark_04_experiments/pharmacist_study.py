...
STUDY_ID = "pharmacist_within_subject_algorithm_6_v1"
CONDITIONS = ("no_tool", "drugeye", "algorithm_6")
...
def prepare_study(args: argparse.Namespace) -> int:
    ...
    summary = {
        ...
        "seed": args.seed,
        "algorithm_condition": "algorithm_6",
        "answer_key_adjudication_status": "pending_two_pharmacists",
        "ethics_status": "approval_or_exemption_required",
    }
...
def validate_study() -> int:
    public_cases = read_csv(STUDY_DATA / "case_manifest.csv")
    answers = read_csv(STUDY_DATA / "answer_key.csv")
    assignments = read_csv(STUDY_ARTIFACTS / "assignments.csv")
    responses = read_csv(STUDY_ARTIFACTS / "response_template.csv")
    case_ids = {row["case_id"] for row in public_cases}
    if len(case_ids) != len(public_cases):
        raise ValueError("case manifest contains duplicate case IDs")
    if {row["case_id"] for row in answers} != case_ids:
        raise ValueError("public manifest and hidden answer key disagree")
    forbidden = {
        "expected_family_key",
        "expected_family_name",
        "ground_truth_raw",
    }
    if forbidden & set(public_cases[0]):
        raise ValueError("public case manifest leaks answer fields")
    for row in public_cases:
        image_path = Path(row["image_path"])
        if not image_path.exists():
            raise ValueError(f"missing study image: {image_path}")
        if file_sha256(image_path) != row["image_sha256"]:
            raise ValueError(f"study image checksum changed: {image_path}")

    assignment_keys = {
        (row["participant_id"], row["case_id"])
        for row in assignments
    }
    if len(assignment_keys) != len(assignments):
        raise ValueError("participant assignments contain duplicate cases")
    if len(responses) != len(assignments):
        raise ValueError("response template does not match assignments")
    for assignment, response in zip(assignments, responses):
        for field in (
            "study_id",
            "trial_id",
            "participant_id",
            ...
            if assignment[field] != response[field]:
                raise ValueError(
                    f"response template changed locked field {field}"
                )
        if assignment["study_id"] != STUDY_ID:
            raise ValueError("assignment uses an unexpected study ID")
        if assignment["condition"] not in CONDITIONS:
            raise ValueError("assignment uses an unknown condition")
        if response["action"].strip():
            raise ValueError("response template already contains outcomes")

    by_case_condition: dict[tuple[str, str], int] = defaultdict(int)
    by_participant_condition: dict[tuple[str, str], int] = defaultdict(int)
    for row in assignments:
        by_case_condition[(row["case_id"], row["condition"])] += 1
        by_participant_condition[
            (row["participant_id"], row["condition"])
        ] += 1
    case_condition_counts = set(by_case_condition.values())
    participant_condition_counts = set(
        by_participant_condition.values()
    )
    if len(case_condition_counts) != 1:
        raise ValueError("cases are not balanced across conditions")
    if len(participant_condition_counts) != 1:
        raise ValueError("participants are not balanced across conditions")
    summary = {
        ...
        "evaluations_per_case_condition": next(
            iter(case_condition_counts)
        ),
        "trials_per_participant_condition": next(
            iter(participant_condition_counts)
        ),
        "answers_hidden_from_manifest": True,
        "image_checksums_verified": len(public_cases),
        "outcomes_present": 0,
    }
    (STUDY_RESULTS / "validation_summary.json").write_text(
        json.dumps(summary, indent=2) + "\n",
        encoding="utf-8",
    )
    print(json.dumps(summary, indent=2))
...
def analyze_study(args: argparse.Namespace) -> int:
    ...
    for row in read_csv(args.responses):
        ...
        if row["condition"] in {"drugeye", "algorithm_6"}:
    ...
    if not args.allow_partial and len(completed) != len(assignments):
        raise ValueError(
            f"expected {len(assignments)} completed trials, found "
            f"{len(completed)}; use --allow-partial only for a declared "
            "interim analysis"
        )
