...
ALGORITHM_6_RESULTS_DIR = BENCHMARK_ROOT / "results/07_algorithm_6"
ALGORITHM_6_ARTIFACTS_DIR = BENCHMARK_ROOT / "artifacts/07_algorithm_6"
ALGORITHM_6_FIGURES_DIR = ALGORITHM_6_RESULTS_DIR / "figures"
...
def write_data_inventory(
    ...
    roles = {
        ...
        "OCR Algorithm 4 results": "Algorithm 4 rankings, safety state, and latency",
        ...
        "Synthetic Algorithm 4 rows": "Algorithm 4 row-level synthetic outcomes",
        ...
        "Algorithm 6 evaluation contract": "Stable case IDs, family-disjoint roles, and cross-fit folds",
        "Algorithm 6 label audit": "P0/P1/P2 review priority and nearest-catalog evidence",
        "Algorithm 6 OCR rows": "Per-case deployed, base, and rejected-policy OCR ranks",
        "Algorithm 6 synthetic rows": "Per-case deployed, base, and rejected-policy synthetic ranks",
        "Algorithm 6 replay metrics": "Top-k and reciprocal-rank metrics by policy, dataset, and split",
        "Algorithm 6 selective gate": "Coverage, accepted correctness, and Wilson lower bounds",
        "Algorithm 6 ranker ablations": "Cross-fit and retrospective feature-group removal metrics",
        "Algorithm 6 learned confusions": "Directional OCR operation counts and discounted costs",
        "Algorithm 6 runtime validation": "Live Python rank parity against cached replay",
    }
    inventory_rows = []
    dictionary_rows = []
        ...
        inventory_rows.append(
            {
                ...
                "role": roles[name],
                "time_coverage": "No timestamp column; Meeting 10 snapshot regenerated 2026-07-24",
            }
        )
...
def plot_ocr_composition(ocr: pd.DataFrame) -> None:
    ...
    axes[1, 1].set_title("D. Algorithm 4 outcomes")
...
def save_algorithm_6_figure(fig: plt.Figure, stem: str) -> None:
    ALGORITHM_6_FIGURES_DIR.mkdir(parents=True, exist_ok=True)
    fig.tight_layout()
    fig.savefig(
        ALGORITHM_6_FIGURES_DIR / f"{stem}.png",
        dpi=180,
        bbox_inches="tight",
        facecolor="white",
    )
...
def build_algorithm_6_report_figures_and_statistics() -> dict[str, Any]:
    summary = json.loads(
        (ALGORITHM_6_RESULTS_DIR / "algorithm_6_summary.json").read_text(
            encoding="utf-8"
        )
    )
    replay_metrics = pd.read_csv(
        ALGORITHM_6_RESULTS_DIR / "algorithm_6_replay_metrics.csv"
    )
    audit = pd.read_csv(
        ALGORITHM_6_ARTIFACTS_DIR / "ocr_label_audit_queue.csv",
        low_memory=False,
    )
    ocr = pd.read_csv(
        ALGORITHM_6_ARTIFACTS_DIR / "algorithm_6_ocr_results.csv.gz",
        low_memory=False,
    )
    synthetic = pd.read_csv(
        ALGORITHM_6_ARTIFACTS_DIR
        / "algorithm_6_synthetic_results.csv.gz",
        low_memory=False,
    )
    abstention = pd.read_csv(
        ALGORITHM_6_RESULTS_DIR / "abstention_metrics.csv"
    )
    runtime = pd.read_csv(
        ALGORITHM_6_RESULTS_DIR / "runtime_replay_validation.csv",
        low_memory=False,
    )

    if len(ocr) != 464 or len(synthetic) != 66_257:
        raise ValueError(
            "Algorithm 6 denominator mismatch: "
            f"{len(ocr)} OCR and {len(synthetic)} synthetic rows"
        )
    if not (
        runtime[["rank_match", "top_1_match", "top_20_match"]]
        .eq(1)
        .all()
        .all()
    ):
        raise ValueError("Algorithm 6 runtime and cached replay do not match")

    ALGORITHM_6_FIGURES_DIR.mkdir(parents=True, exist_ok=True)
    for old_figure in ALGORITHM_6_FIGURES_DIR.glob("*.png"):
        old_figure.unlink()

    paired_groups = [
        (
            "Deployed OCR\nHit@1",
            summary["ocr"]["paired_outcomes"]["deployed_algorithm_6"],
            "hit_at_1",
        ),
        (
            "Deployed OCR\nHit@20",
        ...
        (
            "Rejected gate OCR\nHit@1",
            summary["ocr"]["paired_outcomes"][
                "rejected_rank_1_consensus_gate"
            ],
            "hit_at_1",
        ),
        (
            "Rejected gate synthetic\nHit@1",
            summary["synthetic"]["paired_outcomes"][
                "rejected_rank_1_consensus_gate"
            ],
            "hit_at_1",
        ),
    ]
    paired_labels = [group[0] for group in paired_groups]
    paired_gains = [
        group[1][f"{group[2]}_gains"] for group in paired_groups
    ]
    paired_losses = [
        group[1][f"{group[2]}_losses"] for group in paired_groups
    ]
    ...
    gain_bars = ax.bar(
        ...
        width=0.38,
        color=BLUE,
        label="Cases gained versus Algorithm 5",
    )
    loss_bars = ax.bar(
        x + 0.19,
        paired_losses,
        width=0.38,
        color=ORANGE,
        label="Cases lost versus Algorithm 5",
    )
    ax.bar_label(gain_bars, padding=3)
    ax.bar_label(loss_bars, padding=3)
    ax.set_title("Paired gains and losses determine the Algorithm 6 policy")
    ax.set_ylabel("Paired cases (count)")
    ax.set_xlabel("Policy, dataset, and metric")
    ax.set_xticks(x, paired_labels)
    ax.set_ylim(0, max(paired_losses + paired_gains) * 1.16)
    ax.grid(axis="y", color=LIGHT_GRAY)
    ax.set_axisbelow(True)
    ax.legend(loc="upper left")
    save_algorithm_6_figure(fig, "a6_01_paired_policy_outcomes")

    misses = ocr.loc[ocr["hit_at_20"].eq(0), ["case_id"]].merge(
        audit[["case_id", "audit_priority"]],
        on="case_id",
        validate="one_to_one",
    )
    priorities = ["P0", "P1", "P2"]
    ...
    miss_counts = (
        misses["audit_priority"]
        .value_counts()
        .reindex(priorities, fill_value=0)
    )
    recovered = totals - miss_counts
    fig, ax = plt.subplots(figsize=(9.8, 5.4))
    y = np.arange(len(priorities))
    recovered_bars = ax.barh(
        y,
        recovered,
        color=CYAN,
        label="Expected family inside top 20",
    )
    miss_bars = ax.barh(
        y,
        miss_counts,
        left=recovered,
        color=ORANGE,
        label="Expected family outside top 20",
    )
    for index, (total, missed) in enumerate(zip(totals, miss_counts)):
        ax.text(
            total + 4,
            index,
            f"{missed}/{total} misses ({missed / total:.1%})",
            va="center",
            fontsize=9,
        )
    ax.set_yticks(
        y,
        [
            "P0, likely label or evidence conflict",
            "P1, one-edit catalog conflict",
            "P2, provisional audit-clean",
        ],
    )
    ax.invert_yaxis()
    ax.set_xlim(0, max(totals) * 1.43)
    ax.set_xlabel("Primary OCR pairs (count)")
    ax.set_ylabel("Automated label-audit priority")
    ax.set_title("All deployed Algorithm 6 top-20 misses are P0 or P1")
    ax.grid(axis="x", color=LIGHT_GRAY)
    ax.set_axisbelow(True)
    ax.legend(loc="lower right")
    save_algorithm_6_figure(fig, "a6_02_label_audit_and_misses")

    abstention_labels = {
        "crossfit_development_all": "Development, all",
        "crossfit_development_audit_clean": "Development, P2",
        "retrospective_holdout_all": "Holdout, all",
        "retrospective_holdout_audit_clean": "Holdout, P2",
    }
    abstention = abstention.assign(
        label=abstention["split"].map(abstention_labels)
    )
    fig, axes = plt.subplots(1, 2, figsize=(12.2, 5.4))
    x = np.arange(len(abstention))
    width = 0.38
    coverage_bars = axes[0].bar(
        x - width / 2,
        abstention["coverage"] * 100,
        width,
        color=BLUE,
        label="Coverage",
    )
    accuracy_bars = axes[0].bar(
        x + width / 2,
        abstention["selective_accuracy"] * 100,
        width,
        color=PURPLE,
        label="Accuracy among accepted",
    )
    add_percent_labels(axes[0], coverage_bars)
    add_percent_labels(axes[0], accuracy_bars)
    axes[0].set_ylim(0, 112)
    axes[0].set_ylabel("Percentage of cases (%)")
    axes[0].set_xlabel("Evaluation subset")
    axes[0].set_xticks(x, abstention["label"], rotation=20, ha="right")
    axes[0].set_title("Coverage and selective accuracy")
    axes[0].grid(axis="y", color=LIGHT_GRAY)
    axes[0].legend(loc="lower left")

    correct_bars = axes[1].bar(
        x,
        abstention["correct"],
        color=CYAN,
        label="Accepted and correct",
    )
    error_bars = axes[1].bar(
        x,
        abstention["errors"],
        bottom=abstention["correct"],
        color=ORANGE,
        label="Accepted but wrong",
    )
    for index, row in abstention.iterrows():
        axes[1].text(
            index,
            row["accepted"] + 2,
            f"{int(row['accepted'])} accepted",
            ha="center",
            fontsize=8,
        )
    axes[1].set_ylabel("Accepted cases (count)")
    axes[1].set_xlabel("Evaluation subset")
    axes[1].set_xticks(x, abstention["label"], rotation=20, ha="right")
    axes[1].set_title("Accepted cases split by correctness")
    axes[1].grid(axis="y", color=LIGHT_GRAY)
    axes[1].legend(loc="upper right")
    fig.suptitle(
        "Zero observed gate errors occur only in provisional P2 subsets",
        fontweight="bold",
    )
    save_algorithm_6_figure(fig, "a6_03_selective_gate")

    all_ocr_metrics = replay_metrics.loc[
        replay_metrics["dataset"].eq("ocr")
        & replay_metrics["split"].eq("all")
    ].copy()
    source_labels = {
        "algorithm_6_full": "Complete Algorithm 6",
        "without_top_20_consensus_slot": "Without consensus slot",
        "without_jaro_winkler": "Without Jaro-Winkler",
        "without_match_rating": "Without Match Rating",
        "without_rapidfuzz_wratio": "Without RapidFuzz WRatio",
        "without_nysiis": "Without NYSIIS",
        "without_bm25_plus_char3": "Without BM25Plus char 3-gram",
        "without_symspell_uniform_ed3": "Without SymSpell",
        "without_rapidfuzz_token_sort": "Without token-sort ratio",
        "without_soundex": "Without Soundex",
        "without_dice_char2": "Without Dice char bigram",
    }
    source_ablation = all_ocr_metrics.loc[
        all_ocr_metrics["configuration"].isin(source_labels)
    ].copy()
    source_ablation["label"] = source_ablation["configuration"].map(
        source_labels
    )
    full_misses = int(
        source_ablation.loc[
            source_ablation["configuration"].eq("algorithm_6_full"),
            "outside_top_20",
        ].iloc[0]
    )
    source_ablation["additional_misses"] = (
        source_ablation["outside_top_20"] - full_misses
    )
    source_ablation = source_ablation.sort_values(
        ["additional_misses", "label"],
        ascending=[False, True],
    )
    fig, ax = plt.subplots(figsize=(10.5, 6.0))
    y = np.arange(len(source_ablation))
    bars = ax.barh(
        y,
        source_ablation["additional_misses"],
        color=[
            ORANGE if value > 0 else CYAN
            for value in source_ablation["additional_misses"]
        ],
    )
    ax.bar_label(bars, padding=4)
    ax.set_yticks(y, source_ablation["label"])
    ax.invert_yaxis()
    ax.set_xlim(0, 2.5)
    ax.set_xticks([0, 1, 2])
    ax.set_xlabel("Additional OCR top-20 misses versus full Algorithm 6")
    ax.set_ylabel("One-source removal")
    ax.set_title("Only four candidate sources affect OCR Hit@20 on 464 pairs")
    ax.grid(axis="x", color=LIGHT_GRAY)
    ax.set_axisbelow(True)
    save_algorithm_6_figure(fig, "a6_04_candidate_source_ablation")

    miss_details = ocr.loc[ocr["hit_at_20"].eq(0)].copy()
    mechanism_labels = {
        "multi_edit_ocr_error": "Four or more edits",
        "two_or_three_edit_ocr_error": "Two or three edits",
        "visible_prefix_or_suffix_fragment": "Visible fragment",
        "single_edit_ocr_error": "One edit",
    }
    mechanism_counts = (
        miss_details["error_type"]
        .map(mechanism_labels)
        .value_counts()
    )
    split_counts = miss_details["split"].map(
        {
            "development": "Development",
            "holdout": "Retrospective holdout",
        }
    ).value_counts()
    ...
    for ax, values, title, ylabel in [
        ...
        bars = ax.barh(y, values.values, color=ORANGE)
        ax.bar_label(bars, padding=4)
        ax.set_yticks(y, values.index)
        ax.invert_yaxis()
        ax.set_xlabel("Algorithm 6 top-20 misses (count)")
        ax.set_ylabel(ylabel)
        ax.set_title(title)
        ax.grid(axis="x", color=LIGHT_GRAY)
        ax.set_axisbelow(True)
    fig.suptitle(
        "The 124 remaining OCR misses are concentrated in severe corruption",
        fontweight="bold",
    )
    save_algorithm_6_figure(fig, "a6_05_remaining_failures")

    failure_summary = pd.concat(
        [
            pd.DataFrame(
                {
                    "dimension": "audit_priority",
                    "group": miss_counts.index,
                    "misses": miss_counts.values,
                }
            ),
            pd.DataFrame(
                {
                    "dimension": "error_type",
                    "group": mechanism_counts.index,
                    "misses": mechanism_counts.values,
                }
            ),
            pd.DataFrame(
                {
                    "dimension": "split",
                    "group": split_counts.index,
                    "misses": split_counts.values,
                }
            ),
        ],
        ignore_index=True,
    ...
    write_csv(
        ALGORITHM_6_RESULTS_DIR
        / "algorithm_6_remaining_failure_summary.csv",
        failure_summary.to_dict("records"),
    )

    return {
        "figures": len(list(ALGORITHM_6_FIGURES_DIR.glob("*.png"))),
        "ocr_top_20_misses": int(len(miss_details)),
        "ocr_audit_clean_top_20_misses": int(
            miss_counts.get("P2", 0)
        ),
        "runtime_exact_matches": int(
            runtime[["rank_match", "top_1_match", "top_20_match"]]
            .eq(1)
            .all(axis=1)
            .sum()
        ),
        "synthetic_top_20_misses": int(
            synthetic["hit_at_20"].eq(0).sum()
        ),
    }
...
def build_report_figures_and_statistics(
    ...
    write_data_inventory(
        {
            ...
            "Algorithm 6 evaluation contract": (
                ALGORITHM_6_RESULTS_DIR / "evaluation_contract.csv"
            ),
            "Algorithm 6 label audit": (
                ALGORITHM_6_ARTIFACTS_DIR / "ocr_label_audit_queue.csv"
            ),
            "Algorithm 6 OCR rows": (
                ALGORITHM_6_ARTIFACTS_DIR
                / "algorithm_6_ocr_results.csv.gz"
            ),
            "Algorithm 6 synthetic rows": (
                ALGORITHM_6_ARTIFACTS_DIR
                / "algorithm_6_synthetic_results.csv.gz"
            ),
            "Algorithm 6 replay metrics": (
                ALGORITHM_6_RESULTS_DIR
                / "algorithm_6_replay_metrics.csv"
            ),
            "Algorithm 6 selective gate": (
                ALGORITHM_6_RESULTS_DIR / "abstention_metrics.csv"
            ),
            "Algorithm 6 ranker ablations": (
                ALGORITHM_6_RESULTS_DIR
                / "learned_ranker_ablation_metrics.csv"
            ),
            "Algorithm 6 learned confusions": (
                ALGORITHM_6_RESULTS_DIR
                / "learned_ocr_confusion_costs.csv"
            ),
            "Algorithm 6 runtime validation": (
                ALGORITHM_6_RESULTS_DIR
                / "runtime_replay_validation.csv"
            ),
        }
    )
    ...
    algorithm_6_analysis = (
        build_algorithm_6_report_figures_and_statistics()
    )
    ...
    return {
        ...
        "algorithm_6": algorithm_6_analysis,
        "unsupported_analyses": [
            "training curves: no learned-model training logs in Meeting 10 inputs",
            "cross-system calibration: only Algorithm 6 emits a selective correctness score",
