...
4 & Calculate evidence & Attach ordinary distance, weighted distance, position, edge, family, n-gram, skeleton, phonetic, length, and directional OCR visual evidence. & Two distance-1 candidates can receive different weighted, edge, and retriever-agreement values. A query digit can also explain one candidate letter but not another. \\
...
6 & Bounded correction & Replace the first candidate only under a narrow rule listed in Table~\ref{tab:corrections}. Most rules require the alternative to be strictly closer; the OCR visual rule operates inside an equal-distance tie. & For \code{0IANTA}, \code{OIANDA} can replace \code{SIENTA} because only \code{OIANDA} explains \code{0}$\rightarrow$\code{O}. \\
...
closer alternative. Two bounded exceptions can act inside a tie: a clean
missing-letter relation and a directional OCR digit-to-letter explanation that
only one candidate satisfies. Stage 7 still marks the answer ambiguous.
\end{minipage}}
\end{center}
...
Pure deletion candidate & Query length is at least 5; it is an ordered subsequence of the candidate; exactly one or two missing characters explain the difference; candidate is no farther than the current first result; score gap is at most 0.40. & Every visible character must remain in order, so the rule models omitted unreadable letters rather than arbitrary similarity. & \code{RIVOTRI}$\rightarrow$\code{RIVOTRIL}: the query contains every candidate character except the final L, in the same order. It is eligible only if the competing result is no closer and the score gap stays within 0.40. \\
Directional OCR visual tie & Query contains exactly one digit; alternative is among the first five candidates; ordinary distance equals the current first result; only the alternative gains from a known digit-to-letter mapping; its visual weighted distance is lower; score gap is at most 0.25. & Requiring a unique visual gain prevents the clue from changing order when both candidates explain the digit. Restricting the direction prevents ordinary letters or dosage numbers from being rewritten as drug letters. & \code{0IANTA}: \code{OIANDA} gains from \code{0}$\rightarrow$\code{O}, while \code{SIENTA} does not. For \code{5IGMGREL}, both \code{SIGMAGREL} and \code{SIGAGREL} gain from \code{5}$\rightarrow$\code{S}, so their order remains unchanged. \\
...
\bottomrule
\end{tabularx}
\caption{Equal ordinary distance changes order only for controlled pure-deletion or directional OCR visual evidence; neither rule removes the ambiguity response.}
...
\subsection{Worked Example: \code{0IANTA}}
...
\toprule
\textbf{Evidence} & \textbf{SIENTA} & \textbf{OIANDA} & \textbf{Interpretation} \\
\midrule
Ordinary edit distance & 2.00 & 2.00 & Tie. Ordinary distance cannot identify the better visual explanation. \\
Original weighted distance & 1.70 & \textbf{1.45} & Existing letter-confusion evidence already weakly favors \code{OIANDA}. \\
Directional OCR mapping & none & \textbf{0$\rightarrow$O} & Only \code{OIANDA} explains the observed digit as a visually similar catalog letter. \\
OCR visual distance & 1.70 & \textbf{0.90} & The mapped replacement costs 0.45 instead of 1.00; lower is better. \\
Combined score before correction & \textbf{1.210048} & 1.194095 & The old edge-heavy order leads by only 0.015953, below the 0.25 bound. \\
Deployed display rank & 2 & \textbf{1} & \code{OIANDA} moves first, but the response remains \code{equal\_distance\_ambiguity}. \\
\bottomrule
\end{tabularx}
\caption{The correction uses an observed OCR mechanism rather than a first-character preference or a medicine-name exception.}
\label{tab:oianta-visual-tie}
\end{table}
...
\midrule
\endhead
Current deployed order & Keep full Algorithm 4 score order, then apply the bounded directional OCR visual correction. & \textbf{47.63\%} & 47.98\% & \textbf{46.24\%} \\
...
The directional OCR correction leaves the 464-pair OCR headline score unchanged
because none of its 13 digit-bearing observations contains a correct eligible
tie. On the 66,257-pair synthetic clean core, it fixes 53 top-1 rows and breaks
zero: overall Hit@1 rises from 97.1837\% to 97.2637\%, and holdout Hit@1 rises
from 96.9590\% to 97.0190\%. Hit@20 is unchanged because the corrected families
were already present in the first five candidates.
