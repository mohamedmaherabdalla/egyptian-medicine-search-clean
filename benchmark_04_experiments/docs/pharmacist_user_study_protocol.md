...
Does Algorithm 6 improve correct commercial-family identification and reduce
...
- Conditions: `no_tool`, `drugeye`, and `algorithm_6`.
...
Algorithm 6 and DrugEye receive text, not the image. Under either tool
...
- `drugeye`: show the crop and a frozen DrugEye result captured under the study
  ...
  appears.
- `algorithm_6`: show the crop and the deployed Algorithm 6 candidate list.
...
Algorithm 6, DrugEye, or OCR output.
...
Run the locked preflight before recruitment:

```bash
python3 benchmark_04_experiments/pharmacist_study.py validate
```

The preflight verifies image checksums, answer-key separation, unique trials,
and exact participant-by-condition balance. It also confirms that the response
template contains no outcomes.
...
- Algorithm 6 commit, catalog checksum, deployment URL, and raw outputs.
