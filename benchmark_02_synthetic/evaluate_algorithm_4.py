...
MANUAL_CASES_PATH = DATASET_DIR / "data" / "manual_cases.csv"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--manual", action="store_true", help="Evaluate the adjudicated manual cases CSV.")
    parser.add_argument("--manual-csv", type=Path, default=MANUAL_CASES_PATH)
...
def main() -> int:
    ...
    cases = manual_case_rows(args.manual_csv) if args.manual else read_cases(args.input_csv)
    if args.limit:
...
def manual_case_rows(path: Path = MANUAL_CASES_PATH) -> list[dict[str, Any]]:
    """Load immutable source text plus explicit row-level adjudication."""

    with path.open(newline="", encoding="utf-8-sig") as handle:
        source_rows = list(csv.DictReader(handle))
    rows = []
    for source in source_rows:
        scored = source.get("use_in_primary_score") == "1"
        alternatives = [
            value.strip()
            for value in re.split(
                r"\s+OR\s+",
                source["adjudicated_target"],
                flags=re.IGNORECASE,
            )
            if value.strip()
        ]
        rows.append({
            ...
            "input": source["edited_name"],
            "expected": source["adjudicated_target"],
            "category": "manual_failed_cases",
            "error_type": "manual_observed_typo",
            "case_subcategory": "standard" if scored else "manual_adjudication_excluded",
            "unreadable_continuation": False,
            "difficulty": "EXTREME",
            "danger": "DANGEROUS",
            "scope": "manual",
            "expected_behavior": source["expected_behavior"],
            "collision_with": ";".join(alternatives[1:]),
            "source_base_group": alternatives[0],
            "generator_function": "manual_cases",
            "manual_use_in_primary_score": scored,
            "supplied_right_name": source["supplied_right_name"],
            "maher_notes": source["maher_notes"],
            "adjudication_status": source["adjudication_status"],
            "adjudication_reason": source["adjudication_reason"],
            "duplicate_query_target_pair": source["duplicate_query_target_pair"],
            "supplied_target_occurrences": source["supplied_target_occurrences"],
        })
    return rows
...
def evaluate_algorithm_4_case(case: dict[str, Any]) -> dict[str, Any]:
    ...
    scored_case = bool(case.get(
        "manual_use_in_primary_score",
        case.get("case_subcategory") not in {
            "exact_real_name_collision",
            "catalog_target_absent",
        },
    ))
    expected_distance = expected_edit_distance(case["input"], case["expected"])
    top1_distance = edit_distance(case["input"], top1.get("name", "")) if top1 else 999
    expected_alternatives = [
        value.strip()
        for value in re.split(r"\s+OR\s+", case["expected"], flags=re.IGNORECASE)
        if value.strip()
    ]
    expected_catalog_ids = {
        ALGORITHM_4_CATALOG.rescue_index.family_by_key.get(current_eval.compact_key(value))
        for value in expected_alternatives
    }
    expected_catalog_ids.discard(None)
    exact_query_ids = ALGORITHM_4_CATALOG.rescue_index.exact.get(
        current_eval.compact_key(case["input"]),
        set(),
    )
    exact_query_families = sorted(
        ALGORITHM_4_CATALOG.rescue_index.families[family_id].name
        for family_id in exact_query_ids
    )
    returned_distances = [
        (str(item.get("name") or ""), edit_distance(case["input"], item.get("name", "")))
    ...
    return {
        ...
        "supplied_right_name": case.get("supplied_right_name", case["expected"]),
        "maher_notes": case.get("maher_notes", ""),
        "adjudication_status": case.get("adjudication_status", "not_applicable"),
        "adjudication_reason": case.get("adjudication_reason", ""),
        "duplicate_query_target_pair": case.get("duplicate_query_target_pair", ""),
        "supplied_target_occurrences": case.get("supplied_target_occurrences", ""),
        "expected_target_in_catalog": int(bool(expected_catalog_ids)),
        "query_exact_catalog_families": "|".join(exact_query_families),
        "query_is_other_real_medicine": int(bool(exact_query_ids - expected_catalog_ids)),
        "closer_returned_families": "|".join(
            name for name, distance in returned_distances if distance < expected_distance
        ),
        "equal_distance_returned_families": "|".join(
            name for name, distance in returned_distances
            if distance == expected_distance and current_eval.compact_key(name) not in {
                current_eval.compact_key(value) for value in expected_alternatives
            }
        ),
...
def expected_edit_distance(query: Any, expected: str) -> int:
    """Return the nearest distance when an ambiguous case has valid alternatives."""

    alternatives = re.split(r"\s+OR\s+", expected, flags=re.IGNORECASE)
    return min(edit_distance(query, alternative) for alternative in alternatives)
...
def write_summary(path: Path, rows: list[dict[str, Any]], elapsed: float, args: argparse.Namespace) -> None:
    input_path = (args.manual_csv if args.manual else args.input_csv).resolve()
