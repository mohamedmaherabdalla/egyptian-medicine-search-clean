#!/usr/bin/env python3
"""Evaluate Algorithm 5 on testing dataset v2 or the manual failed cases."""
...
DEFAULT_OUTPUT_PREFIX = "algorithm_5"
DEFAULT_CASE_OUTPUT = DATASET_DIR / "artifacts" / "01_full_benchmark" / "algorithm_5_cases.csv"
DEFAULT_CHUNK_SIZE = 200
DEFAULT_MAX_WORKERS = 8
CONFIDENT_ALGORITHM_5_STATUSES = {"high_confidence", "medium_confidence"}
...
ALGORITHM_5_CATALOG: algorithm5.Algorithm5Catalog | None = None
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate Algorithm 5 on V2 data.")
...
def main() -> int:
    ...
    failures = v2_eval.failure_samples(rows, "algorithm_5")
    ...
    LOGGER.info("wrote Algorithm 5 report: %s", paths["report"])
    return 0


def initialize_global_state() -> None:
    """Prepare Algorithm 5 and the relevance index used by V2 metrics."""

    global ALGORITHM_5_CATALOG
    ALGORITHM_5_CATALOG = algorithm5.prepare_catalog()
    # This relevance index is evaluator-only. Algorithm 5 itself does not use
...
def evaluate_chunk(cases: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [evaluate_algorithm_5_case(case) for case in cases]


def evaluate_algorithm_5_case(case: dict[str, Any]) -> dict[str, Any]:
    if ALGORITHM_5_CATALOG is None:
        raise RuntimeError("ALGORITHM_5_CATALOG is not initialized")
    ...
    response = algorithm5.search_catalog(ALGORITHM_5_CATALOG, query_request, v2_eval.TOP_K_RESULTS)
    ...
    top1_confident = bool(
        top1
        and status in CONFIDENT_ALGORITHM_5_STATUSES
    ...
    behavior_success = algorithm_5_behavior_success(case, first_rank, status, bool(results))
...
def algorithm_5_behavior_success(case: dict[str, Any], first_rank: int, status: str, has_results: bool) -> bool:
    if case["expected_behavior"] == "match":
        return first_rank <= v2_eval.TOP_K_RESULTS
    if case["expected_behavior"] == "ambiguous":
        return status not in CONFIDENT_ALGORITHM_5_STATUSES and has_results
    return (not has_results) or status not in CONFIDENT_ALGORITHM_5_STATUSES
...
def edit_distance(left: Any, right: Any) -> int:
    ...
    if not left_key or not right_key:
        return 999
    return int(algorithm5.damerau(left_key, right_key, weighted=False))


def expected_variant_group(expected: str) -> str:
    """Resolve the expected family to Algorithm 5's catalog variant group."""

    if ALGORITHM_5_CATALOG is None:
        return current_eval.normalize_search(expected)
    key = current_eval.compact_key(expected)
    family_id = ALGORITHM_5_CATALOG.rescue_index.family_by_key.get(key)
    if family_id is None:
        return current_eval.normalize_search(expected)
    family = ALGORITHM_5_CATALOG.rescue_index.families[family_id]
...
def write_report(path: Path, rows: list[dict[str, Any]], metrics: list[dict[str, Any]], elapsed: float, args: argparse.Namespace) -> None:
    ...
    mistake_metrics = metric_rows_by_mistake_type(rows)
    lines = [
        "# Algorithm 5 Benchmark Report",
        "",
        "Algorithm 5 = evidence-guided family retrieval, bounded reranking, and conservative clarification.",
        "",
        "## Run",
        "",
        f"- Cases: `{len(rows):,}`",
